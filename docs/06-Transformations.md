...
# تبدیلات هندسی در OpenGL - درس گرافیک کامپیوتری
...
### تمرین 1: تبدیلات پایه
1. یک مکعب ایجاد کنید که حول محور Y بچرخد
2. با کلیدهای جهت‌دار، مکعب را در صفحه XY جابجا کنید
3. با کلیدهای + و -، اندازه مکعب را تغییر دهید
4. یک هرم ایجاد کنید که همزمان حول چند محور بچرخد

### تمرین 2: تبدیلات پیشرفته
1. یک روبات با چندین مفصل ایجاد کنید که هر بخش آن به صورت مستقل بچرخد
2. یک سیستم خورشیدی با سیارات و قمرها پیاده‌سازی کنید
3. یک دوربین اول شخص با کنترل‌های WASD و ماوس ایجاد کنید
4. یک سیستم ذره‌ای با ذرات در حال حرکت و چرخش پیاده‌سازی کنید

### تمرین 3: تبدیلات ترکیبی
1. یک کاراکتر با حرکت‌های پیچیده (مثل راه رفتن، دویدن، پریدن) ایجاد کنید
2. یک سیستم فیزیکی ساده با گرانش و برخورد پیاده‌سازی کنید
3. یک سیستم انیمیشن با استفاده از تبدیلات میانی (interpolation) ایجاد کنید
4. یک سیستم دوربین با قابلیت‌های مختلف (اول شخص، سوم شخص، مداربینی) پیاده‌سازی کنید

## مفاهیم پیشرفته

### 1. تبدیلات میانی (Interpolation)
- **خطی (Linear)**: تغییر یکنواخت بین دو حالت
- **کروی (Spherical)**: برای چرخش‌های روان
- **بزیه (Bézier)**: برای مسیرهای منحنی
- **اسپلاین (Spline)**: برای مسیرهای پیچیده

### 2. تبدیلات با محدودیت (Constrained Transformations)
- **محدودیت‌های مفصل (Joint Constraints)**: برای مدل‌سازی حرکت‌های واقع‌گرایانه
- **محدودیت‌های فیزیکی (Physics Constraints)**: برای شبیه‌سازی قوانین فیزیکی
- **محدودیت‌های مسیر (Path Constraints)**: برای حرکت در مسیرهای مشخص

### 3. تبدیلات برای انیمیشن
- **انیمیشن اسکلتی (Skeletal Animation)**: برای کاراکترهای پیچیده
- **انیمیشن مورفینگ (Morphing)**: برای تغییر شکل نرم بین حالت‌های مختلف
- **انیمیشن ذره‌ای (Particle Animation)**: برای افکت‌های ویژه
